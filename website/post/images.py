...
from ..database.models import Image,User,Video
...
#creates and saves video
def createvideo(file):
    if file:
        filename=secure_filename(file.filename)
        file_ext = os.path.splitext(filename)[1]
        if file_ext not in app.config['UPLOAD_EXTENSIONS']:
            ...
            filename=str(uuid.uuid1())+"_"+filename
            file.save(os.path.join(basedir,app.config['UPLOAD_VIDEO'],filename))
            return filename

def deletevideo(videoid):
    video=Video.query.filter_by(id=videoid).first()
    if video:
        os.remove(os.path.join(basedir,app.config['UPLOAD_VIDEO'],video.video))
